...
public function get_pipeline_leads($pipeline_id, $where = [])
{
    ...
    $this->db->select('mpl.*, l.name, l.email, l.phonenumber, s.name as stage_name')
...
{
    return $this->db->where('pipeline_id', $pipeline_id)->get('tblmulti_pipeline_leads')->result_array();
}

public function update_kanban_lead_stage($lead_id, $stage_id)
{
    return $this->db->where('id', $lead_id)->update('tblmulti_pipeline_leads', ['stage_id' => $stage_id]);
}
...
public function get_stages($pipeline_id = null)
{
    ...
    public function get_leads($id = null, $where = []) {
        if ($id !== null) {
            // Verificar se o parâmetro $id é válido e seguro
            if (!is_numeric($id) || $id < 1) {
        ...
        $this->db->from(db_prefix() . 'multi_pipeline_leads');
